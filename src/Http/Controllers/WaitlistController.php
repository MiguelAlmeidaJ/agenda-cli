...
final class WaitlistController
{
    public function store(string $slug): void
    {
        ...
        $insert->execute([
            ...
            'service' => $serviceId,
            'customer' => $customerId,
            'provider' => $providerId > 0 ? $providerId : null,
            'date' => $date,
            'period' => $period,
    ...
    public function index(): void
    {
        ...
        $stmt = Database::connection()->prepare(
            ...
            . 'u.name AS provider_name FROM waitlist_entries w '
            . 'JOIN services s ON s.id = w.service_id JOIN customers c ON c.id = w.customer_id '
    ...
    public function status(string $id): void
    {
        ...
        $stmt = Database::connection()->prepare('UPDATE waitlist_entries SET status = :status WHERE id = :id AND establishment_id = :establishment');
