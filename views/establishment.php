  ...
  <div class="col-lg-7">
    ...
    <div class="d-grid gap-3" id="public-service-list">
      <?php foreach ($services as $service): ?>
        ...
        <button
          type="button"
          class="public-service-card text-start"
          data-service-id="<?= $serviceId ?>"
          data-service-name="<?= e($service['name']) ?>"
          data-service-price="<?= e(number_format((float) $service['price'], 2, ',', '.')) ?>"
          data-service-duration="<?= (int) $service['duration_minutes'] ?>"
          aria-pressed="false"
        >
          <span class="public-service-main">
            <span class="public-service-title"><?= e($service['name']) ?></span>
            <?php if (!empty($service['description'])): ?>
              <span class="public-service-description"><?= e($service['description']) ?></span>
            <?php endif; ?>
            <span class="public-service-meta"><i class="bi bi-clock me-1"></i><?= (int) $service['duration_minutes'] ?> min</span>
          </span>
          <span class="public-service-side">
            <strong>R$ <?= e(number_format((float) $service['price'], 2, ',', '.')) ?></strong>
            <span class="public-service-select">Selecionar <i class="bi bi-arrow-right"></i></span>
          </span>
        </button>
      <?php endforeach; ?>
      ...
      <?php if ($services === []): ?>
        <div class="card empty-state">
          <div class="card-body p-5 text-center text-muted-app">Este estabelecimento ainda não possui serviços disponíveis para agendamento.</div>
        </div>
      <?php endif; ?>
    </div>
  </div>
    ...
    <div class="card public-booking-card sticky-lg-top" id="booking" style="top: 88px">
      <div class="card-body p-4 p-xl-5">
        <div class="d-flex align-items-start justify-content-between gap-3 mb-4">
          <div>
            <div class="small text-muted-app mb-1">Reserve em poucos passos</div>
            <h2 class="h4 mb-0">Agendar horário</h2>
          </div>
        ...
        <form method="post" action="<?= e(url('/estabelecimentos/' . $establishment['slug'] . '/agendar')) ?>" id="booking-form">
          ...
          <div class="booking-selected-service mb-4" id="selected-service-summary">
            <div class="booking-step-number">1</div>
            <div>
              <strong>Escolha um serviço</strong>
              <small>Clique em um serviço ao lado para começar.</small>
            </div>
          </div>
          ...
          <div class="booking-section is-disabled" id="provider-section">
            <div class="booking-section-heading">
              ...
              <div>
                ...
                <small>Quem você prefere para o atendimento?</small>
              </div>
            </div>
            <div class="provider-public-list mt-3" id="provider-list">
              <span class="small text-muted-app">Escolha um serviço primeiro.</span>
            </div>
          </div>
          ...
          <div class="booking-section is-disabled" id="date-section">
            <div class="booking-section-heading">
              ...
              <div>
                ...
                <small>Mostramos apenas horários realmente livres.</small>
              </div>
            </div>

            <div class="booking-date-strip mt-3" id="quick-dates">
              <?php foreach ($quickDates as $dateItem): ?>
                <button type="button" class="booking-date-option" data-date="<?= e($dateItem['value']) ?>">
                  <span><?= e($dateItem['weekday']) ?></span>
                  <strong><?= e($dateItem['day']) ?></strong>
                  <small><?= e($dateItem['month']) ?></small>
                </button>
              <?php endforeach; ?>
            </div>

            <div class="mt-3">
              <label class="form-label small text-muted-app" for="date">Outra data</label>
              <input class="form-control" type="date" name="date" id="date" min="<?= e($today) ?>" required>
            </div>

            <div class="mt-4">
              <div class="d-flex justify-content-between align-items-center mb-2">
                <label class="form-label mb-0">Horários disponíveis</label>
                <span class="small text-muted-app" id="slot-count"></span>
              </div>
              <div id="slots" class="slot-grid public-slot-grid">
                <span class="small text-muted-app">Escolha profissional e data.</span>
              </div>
            </div>
          </div>

          <div class="booking-section booking-notes-section">
            <label class="form-label">Observação <span class="text-muted-app fw-normal">(opcional)</span></label>
            <textarea class="form-control" rows="2" name="notes" maxlength="500" placeholder="Algo que o estabelecimento precisa saber?"></textarea>
          </div>

          <div class="booking-summary d-none" id="booking-summary">
            <div>
              <small>Seu agendamento</small>
              <strong id="summary-main"></strong>
              <span id="summary-detail"></span>
            </div>
            <strong id="summary-price"></strong>
          </div>

          <button class="btn btn-dark btn-lg w-100" type="submit" id="booking-submit" disabled>
            <i class="bi bi-calendar2-check me-2"></i>Confirmar agendamento
          </button>

          <?php if (!$user): ?>
            <div class="small text-muted-app text-center mt-3"><i class="bi bi-person me-1"></i>Você fará login ou criará sua conta para concluir.</div>
          <?php elseif (($user['role'] ?? null) !== 'client'): ?>
            <div class="small text-muted-app text-center mt-3">O agendamento público é destinado a contas de cliente.</div>
          <?php endif; ?>
        </form>
      </div>
    </div>
...
dateInput.addEventListener('change', () => {
  dateButtons.forEach(item => item.classList.toggle('active', item.dataset.date === dateInput.value));
  resetTime();
  loadSlots();
});
...
function renderProviders() {
  ...
  if (!people.length) {
    providerList.innerHTML = '<div class="booking-inline-alert"><i class="bi bi-exclamation-circle"></i><span>Este serviço está temporariamente sem profissional disponível.</span></div>';
    dateSection.classList.add('is-disabled');
    return;
  }

  providerList.innerHTML = people.map(person => `
    <button type="button" class="provider-public-option" data-provider-id="${person.id}" data-provider-name="${escapeHtml(person.name)}">
      <span class="provider-avatar">${escapeHtml(person.name.charAt(0).toUpperCase())}</span>
      <span><strong>${escapeHtml(person.name)}</strong><small>${person.role === 'owner' ? 'Responsável pelo estabelecimento' : 'Profissional'}</small></span>
      <i class="bi bi-check-circle-fill"></i>
    </button>
  `).join('');

  [...providerList.querySelectorAll('.provider-public-option')].forEach(button => {
    button.addEventListener('click', () => selectProvider(button));
  });

  if (people.length === 1) {
    selectProvider(providerList.querySelector('.provider-public-option'));
  }
}
...
function selectProvider(button) {
  ...
  selectedProvider = {id: button.dataset.providerId, name: button.dataset.providerName};
  dateSection.classList.remove('is-disabled');
  resetTime();
  updateSummary();
...
async function loadSlots() {
  if (!serviceInput.value || !employeeInput.value || !dateInput.value) {
    slots.innerHTML = '<span class="small text-muted-app">Escolha profissional e data.</span>';
    slotCount.textContent = '';
    return;
  }

  slots.innerHTML = '<span class="booking-loading"><span class="spinner-border spinner-border-sm" aria-hidden="true"></span> Buscando horários livres...</span>';
  slotCount.textContent = '';
  const query = new URLSearchParams({service_id: serviceInput.value, employee_id: employeeInput.value, date: dateInput.value});
  ...
  try {
    ...
    if (!data.slots.length) {
      slots.innerHTML = '<div class="booking-inline-alert w-100"><i class="bi bi-calendar-x"></i><span>Não há horários livres nessa data. Tente outro dia.</span></div>';
      return;
    }

    slotCount.textContent = `${data.slots.length} horário${data.slots.length === 1 ? '' : 's'}`;
    slots.innerHTML = data.slots.map((time, index) => `<button type="button" class="public-slot" data-time="${time}">${time}</button>`).join('');
    [...slots.querySelectorAll('.public-slot')].forEach(button => button.addEventListener('click', () => selectTime(button)));
  } catch (_) {
    slots.innerHTML = '<div class="booking-inline-alert booking-inline-alert-danger w-100"><i class="bi bi-exclamation-triangle"></i><span>Não foi possível carregar os horários. Tente novamente.</span></div>';
  }
}

function selectTime(button) {
  [...slots.querySelectorAll('.public-slot')].forEach(item => item.classList.toggle('active', item === button));
...
function resetTime() {
  ...
  if (employeeInput.value && dateInput.value) {
    slots.innerHTML = '<span class="small text-muted-app">Carregando disponibilidade...</span>';
  } else {
    slots.innerHTML = '<span class="small text-muted-app">Escolha profissional e data.</span>';
  }
  slotCount.textContent = '';
...
function updateSummary() {
  if (!selectedService || !selectedProvider || !dateInput.value || !timeInput.value) {
    summary.classList.add('d-none');
    submitButton.disabled = true;
    return;
  }
  ...
  summaryMain.textContent = `${day}/${month} às ${timeInput.value}`;
  summaryDetail.textContent = `${selectedService.name} · ${selectedProvider.name}`;
  summaryPrice.textContent = `R$ ${selectedService.price}`;
