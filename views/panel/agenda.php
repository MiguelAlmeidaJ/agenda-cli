  ...
  <div class="d-flex flex-wrap justify-content-between align-items-end gap-3">
    ...
    <div class="d-flex flex-wrap gap-2 align-items-center">
      ...
      <form method="get" action="<?= e(url('/painel/agenda')) ?>" class="d-flex gap-2">
        ...
        <button class="btn btn-outline-dark btn-sm" type="submit">Ir</button>
      </form>
    </div>
  </div>
</section>
    ...
    <section class="agenda-provider-column">
      <div class="agenda-provider-header">
        <div class="d-flex align-items-center gap-2">
          ...
          <div><div class="fw-semibold"><?= e($provider['name']) ?></div><div class="small text-muted-app"><?= count($items) ?> atendimento(s)</div></div>
        </div>
      </div>
      ...
      <div class="agenda-provider-body">
        <?php foreach ($items as $appointment): ?>
          <a class="agenda-appointment-card text-decoration-none" href="<?= e(url('/painel/agendamentos/' . $appointment['id'] . '/editar')) ?>">
            <div class="agenda-appointment-time"><strong><?= e(date('H:i', strtotime((string) $appointment['starts_at']))) ?></strong><span><?= e(date('H:i', strtotime((string) $appointment['ends_at']))) ?></span></div>
            <div class="agenda-appointment-content">
              <div class="d-flex justify-content-between gap-2 align-items-start"><div class="fw-semibold text-dark"><?= e($appointment['client_name']) ?></div><span class="status-pill status-<?= e($appointment['status']) ?>"><?= e($statusLabels[$appointment['status']] ?? $appointment['status']) ?></span></div>
        ...
        <?php if ($items === []): ?><div class="agenda-empty-column"><i class="bi bi-calendar2"></i><span>Sem atendimentos</span></div><?php endif; ?>
      </div>
    </section>
  <?php endforeach; ?>

  <?php if ($providers === []): ?><div class="card empty-state w-100"><div class="card-body p-5 text-center text-muted-app">Nenhum profissional ativo encontrado.</div></div><?php endif; ?>
</div>
